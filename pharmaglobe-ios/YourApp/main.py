    ...
    def handle_login(self, method, value=None):
        ...
        def complete_login(dt):
            ...
            if os.environ.get("PHARMAGLOBE_AUTO_SCROLL") == "1":
                def do_scroll(dt):
                    print("AUTO-LOGIN: scrolling home_scroll to 0.65")
                    if hasattr(self.root, "ids") and "home_scroll" in self.root.ids:
                        self.root.ids.home_scroll.scroll_y = 0.65
                        print("AUTO-LOGIN: scroll_y set to 0.65")
                Clock.schedule_once(do_scroll, 4.0)
